...
　  close(f)
    ...
    for k = 1 : instance_num
        ...
        p = InstanceType("", "", 0, Vector{String}[], 0, 0, 0, 0, 
    ...
    for k = 1 : material_num
        ...
        p = MaterialType("", 0.0, 0.0, 0.0, zeros(Float64, 0,0), 0, 0.0, zeros(Float64, 0,0), 0.0, zeros(1,1))
        push!(MATERIAL,p)        
...
#                  BC[k].value{ii} = 0;
 ...
 #                 BC[k].value{ii} = value;
    ...
    #MODEL = ModelType([])
    #push!(MODEL.PART, PART)

    MODEL = ModelType(PART, INSTANCE, NSET, AMPLITUDE, MATERIAL, BC, IC, 
