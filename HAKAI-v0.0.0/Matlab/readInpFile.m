...
% along with this program. If not, see http://www.gnu.org/licenses/.
  ...
  for k = 1 : nPart
      ...
      for i = part_index(k) : n
        if strfind(lines{i}, '*Solid Section') > 0
          s = replace(lines{i},' ', '');
          ss = split( s,',');
          for j = 1 : length(ss)
              sss = ss{j};
  ...
  for i = 1 : length(INSTANCE) %length(PART)
      part_id = INSTANCE(i).part_id;
       ...
       %mat_id = ones(PART(i).nElement,1) * PART(i).material_id;
  ...
  for k = 1 : ic_num
      ...
      for i = index+1 : n
        ...
        if strfind(IC(k).Nset_name, '.') > 0
            ...
            for j = 1 : length(PART(part_id).NSET)
                if strcmp(PART(part_id).NSET(j).name, nset_name)
                    %nodes_j = PART(part_id).NSET(j).nodes + PART(part_id).node_offset;
                    nodes_j = PART(part_id).NSET(j).nodes + INSTANCE(instance_id).node_offset;
                    nodes = [nodes  nodes_j];
                    break
                end
            ...
            for j = 1 : nset_num
                if strcmp(IC(k).Nset_name, NSET(j).name)
                    %nodes_j = NSET(j).nodes + PART(NSET(j).part_id).node_offset;
                    nodes_j = NSET(j).nodes + INSTANCE(NSET(j).instance_id).node_offset;
                    nodes = [nodes nodes_j];
                    break
                end
